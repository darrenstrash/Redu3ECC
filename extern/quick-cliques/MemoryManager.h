...
static void* MallocWithCheck(size_t x)
{
    ...
    #else
    void* retvalue = malloc(alloc_max(x,1));
    #endif
...
static void* CallocWithCheck(size_t x, size_t y)
{
    ...
    #else
    void* retvalue = calloc(alloc_max(x,1),alloc_max(y,1));
    #endif
...
#define Free(x) free(x)
    ...
    #define Malloc(x) malloc(alloc_max(x,1))
    #define Calloc(x,y) calloc(alloc_max(x,1),alloc_max(y,1))
    #define Free(x) free(x)
