...
def GetEnvironment():
  ...
  opts.Add('program', 'program or interface to compile', 'redu3ecc')
...
env.Append(CPPPATH=['../extern/quick-cliques/'])
...
env.Append(CPPPATH=[GUROBI_HOME + '/include/'])
env.Append(LIBPATH=[GUROBI_HOME + '/lib/'])
...
if "clang" in env['CC'] or "clang" in env['CXX']:
        if env['variant'] == 'optimized':
          env.Append(CXXFLAGS = '-DNDEBUG -Wall -funroll-loops -O3 -std=c++20')
          env.Append(CCFLAGS  = '-O3  -DNDEBUG -funroll-loops')
        elif env['variant'] == 'optimized_output':
          # A little bit more output on the console
          env.Append(CXXFLAGS = ' -DNDEBUG -funroll-loops -Wall -O3 -std=c++20')
          env.Append(CCFLAGS  = '-O3  -DNDEBUG -DKAFFPAOUTPUT')
        else:
          env.Append(CXXFLAGS = ' -g -DDEBUG -Wall -funroll-loops -O0 -std=c++20')
          env.Append(CCFLAGS  = ' -g -O0 -DDEBUG -funroll-loops')
          if SYSTEM != 'Darwin':
        ...
        if env['variant'] == 'optimized':
          env.Append(CXXFLAGS = '-DNDEBUG -Wall -funroll-loops -O3 -std=c++20 -fpermissive')
          env.Append(CCFLAGS  = '-O3  -DNDEBUG -funroll-loops -fpermissive')
        elif env['variant'] == 'optimized_output':
          # A little bit more output on the console
          env.Append(CXXFLAGS = ' -DNDEBUG -funroll-loops -Wall -O3 -std=c++20 -fpermissive')
